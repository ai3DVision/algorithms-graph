...
def montaBolas(v1,v2,kMax,arestasUC):
	...
	with ProcessPoolExecutor(max_workers=1) as executor:
		...
		if(arestasUC):
			futures.append(executor.submit(subtreePattern.criaArvoresComRetornos, dictG, v1, kMax))
			futures.append(executor.submit(subtreePattern.criaArvoresComRetornos, dictG, v2, kMax))
	...
	for bolasV1V2 in bolas:
		for kB,vB in bolasV1V2.iteritems():
			for k,v in vB.iteritems():
				...
				futures.append(executor.submit(editDistance2.degreeSequenceEditDistanceComArvores,v['bolaV1'],v['bolaV2'],kB[0],kB[1],k))
				del v
